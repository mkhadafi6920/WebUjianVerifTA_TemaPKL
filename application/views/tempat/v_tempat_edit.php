<!-- isi -->

<div class="container m-5  ">
	<?php foreach($tampil as $data){ ?>
	<form class="mb-4" action="<?php echo base_url('c_tempat/simpanedit');?>" method="post">
	



		<div class="mb-3">
			<label for="nama" class="form-label">Nama Tempat</label>
			<input type="text" value="<?php echo $data->nama_tempat?>" name="nama" class="form-control" id="nama">
            <input type="hidden" name="id" value="<?php echo $data->id_tempat?>">
			<div id="nama" class="form-text text-danger"><?php echo form_error('nama'); ?></div>
		</div>
		<div class="mb-3">
			<label for="nama" class="form-label">Alamat Tempat</label>
			<input type="text" value="<?php echo $data->alamat_tempat?>" name="alamat" class="form-control" id="alamat">
			<div id="alamat" class="form-text text-danger"><?php echo form_error('alamat'); ?></div>
		</div>

		

		<button type="submit" class="btn btn-primary me-3">Simpan</button>

        <a href="<?php echo base_url('c_tempat') ?> " class="btn btn-danger me-3">Batal edit</a> 
	</form>
	<?php } ?>
</div>
<!-- akhir isi -->
